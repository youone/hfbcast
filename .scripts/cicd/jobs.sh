...
# written for node:18.17.1
    ...
    npx --node-options=--inspect -y -p `ls *.tgz` -c `cat package/name.txt`-test
    rm -f `ls *.tgz`
}
...
deploy-image() {
# written for rocky8
    IMAGE_NAME=$CI_REGISTRY/dspdf/hfbcast:latest
    printf "\n\033[0;32mDEPLOYING IMAGE $IMAGE_NAME\033[0m \n"
    chmod 600 $SSHKEY_APP_SERVER
    ssh -o StrictHostKeyChecking=no -i $SSHKEY_APP_SERVER dspdf@dspdf.sys.utv ./deploy_image.sh $IMAGE_NAME hfbcast
}
